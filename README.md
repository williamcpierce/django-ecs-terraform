...
1. Build the Django and Nginx Docker images and push them up to ECR:
    ...
    ```sh
    $ cd app
    $ docker buildx build --platform linux/amd64 --tag <AWS_ACCOUNT_ID>.dkr.ecr.us-west-1.amazonaws.com/django-app:latest .
    $ docker push <AWS_ACCOUNT_ID>.dkr.ecr.us-west-1.amazonaws.com/django-app:latest
    $ cd ..

    $ cd nginx
    $ docker buildx build --platform linux/amd64 --tag <AWS_ACCOUNT_ID>.dkr.ecr.us-west-1.amazonaws.com/nginx:latest .
